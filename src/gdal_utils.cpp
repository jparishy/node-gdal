...
NAN_METHOD(Utils::vectorTranslate)
{
	...
	char *args[] = { (char *) "--version" };
	opts = GDALVectorTranslateOptionsNew(args, NULL);

	GDALDataset *pahSrcDS[] = { ds->getDataset() };
	GDALDataset *outDS = (GDALDataset *)GDALVectorTranslate(NULL, NULL, 1, (GDALDatasetH *)pahSrcDS, opts, &error);
